...
class Temporal_evo(nn.Module):
    def __init__(self, channel_in, channel_hid, N_T, h, w, incep_ker=[3, 5, 7, 11], groups=8):
        ...
        self.mamaba = MambaBlock(self.config)

    def forward(self, x):
        B, T, C, H, W = x.shape #
        bias = x
        x = x.reshape(B, T * C, H, W) #B d H W
        ...
        # Mamba Domain
        B, D, H, W = z.shape # b t c h w # 
        # print("z shape", z.shape)
        L = H * W # H*W = L （p=1）（H）/ p * W / p (p=1)
        z = z.permute(0, 2, 3, 1) # B, H, W, D
        z = z.view(B, L, D) # L, D
        # for blk in self.blocks:
        #     z = blk(z)
        z = self.mamaba(z) # Latent Mamba
...
class Mamba_model(nn.Module):
    def __init__(self, shape_in, hid_S=32, hid_T=64, N_S=2, N_T=2, incep_ker=[3,5,7,11], groups=1):
...
if __name__ == '__main__':
    ...
    model = Mamba_model(shape_in=(10, 1, 64, 64))  # Move the model to GPU
    rec_feature = model(inputs)
    print("Output shape", rec_feature.shape)